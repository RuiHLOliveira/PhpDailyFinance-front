...
export default {
  ...
  emits: ['reloadListaTipoMovimentos'],
  props: {
    exibirModal: Boolean,
  },
  methods: {
    ...
    fecharModal() {
      ...
      if(this.needReload == true) {
        console.log('reload');
        this.$emit('reloadListaTipoMovimentos', [])
      }
      this.resetFields();
    },
    criarTipoMovimento() {
      ...
      let requestData = {
        'url': config.serverUrl + '/tipomovimentos',
      ...
      Request.fetch(requestData).then(([response, data]) => {
        ...
        this.notify('Tipo Movimento criada!');
      }).catch((error) => {
        console.error(error);
        this.busy = false;
